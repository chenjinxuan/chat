...
<seelog type="sync" minlevel="debug">
    ...
    <formats>
        <format id="standard" format="[%Level] [%RelFile %Line] [%Date %Time] [%Msg]%n" />
    </formats>
</seelog>
`
)
...
func init() {
	_logger, _ := seelog.LoggerFromConfigAsBytes([]byte(DefaultSeelogConfig))
	// for custmizing the File and Line but not show the current file
	_logger.SetAdditionalStackDepth(1)
	//seelog.ReplaceLogger(_logger)
	logger = _logger
}
